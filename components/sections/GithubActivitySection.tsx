...
const GitHubActivitySection = () => {
    ...
    return (
        <ScrollAnimationWrapper>
            <section id="github-activity" className="w-full py-12 md:py-16">
                <div className="container mx-auto px-4">
                    ...
                    <div className="bg-[#161b22] p-4 md:p-6 rounded-xl border border-gray-700 shadow-xl mb-6 md:mb-8 max-w-5xl mx-auto">
                        <h3 className="text-lg md:text-xl font-bold mb-3 md:mb-4">Aktivitas GitHub</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-5xl mx-auto">
                        ...
                        <div className="md:col-span-2 bg-[#161b22] p-4 md:p-6 rounded-xl border border-gray-700 shadow-xl">
                            <h3 className="text-lg md:text-xl font-bold mb-3 md:mb-4">Repository Terbaru</h3>

                            {isLoading ? (
                                <div className="flex items-center justify-center h-36 md:h-48">
                                    <div className="animate-spin rounded-full h-8 w-8 md:h-12 md:w-12 border-t-2 border-b-2 border-blue-400"></div>
                                </div>
                            ) : repos.length > 0 ? (
                                <div className="space-y-3 md:space-y-4">
                                    {repos.map((repo) => (
                                        <div key={repo.id} className="p-3 md:p-4 rounded-lg bg-[#21262d] hover:bg-[#30363d] transition-colors">
                                ...
                                <div className="bg-[#21262d] rounded-lg p-4 text-center">
                                    <p className="text-gray-300">Tidak ada repository yang ditemukan.</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-[#161b22] p-4 md:p-6 rounded-xl border border-gray-700 shadow-xl">
                            <h3 className="text-lg md:text-xl font-bold mb-3 md:mb-4">Statistik GitHub</h3>

                            <div className="space-y-3 md:space-y-4">
                                <div className="bg-[#21262d] p-3 md:p-4 rounded-lg">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs md:text-sm text-gray-300">Total Repositories</span>
                                        <span className="font-semibold text-sm md:text-base">{stats.repos}</span>
                                    </div>
                                </div>

                                <div className="bg-[#21262d] p-3 md:p-4 rounded-lg">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs md:text-sm text-gray-300">Recent Activity</span>
                                        <span className="font-semibold text-xs md:text-sm text-green-400">Active</span>
                                    </div>
                                </div>

                                <div className="mt-4 pt-4 border-t border-gray-700">
                                    <h4 className="text-sm md:text-base font-semibold mb-3">Top Languages:</h4>
                                    <div className="space-y-2">
                                        {['JavaScript', 'PHP', 'HTML', 'CSS', 'TypeScript'].map((lang, index) => (
                                            <div key={lang} className="flex items-center space-x-2">
                                                <span
                                                    className="w-3 h-3 rounded-full"
                                                    style={{ backgroundColor: languageColors[lang] }}
                                                ></span>
                                                <span className="text-xs md:text-sm flex-1">{lang}</span>
